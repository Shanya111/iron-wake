...
# и так уже случилось достаточно. Оставлены пять опорных точек: выключено, каждый
# фильтр по отдельности и два лучших сочетания. Между ними видно и направление
# («строже → лучше сделка и реже сигналы»), и цену выбора.
#
...
#
# ЧЕСТНАЯ ОГОВОРКА К СПИСКУ. Две строки из пяти «доминируемы»: есть прогнанный вариант,
# который даёт И БОЛЬШЕ сигналов, И лучшую среднюю сделку одновременно.
#   • «риск 0.5» (28/мес, +0.084) бьётся вариантом «вход 0.1» (31/мес, +0.094);
#   • «вход 0.075 + риск 0.5» (13/мес, +0.164) бьётся строкой «+ риск 0.75»
#     (19/мес, +0.179) — она прямо в списке ниже.
# Оставлены сознательно: обе были в таблице, по которой принималось решение, и обе
# показывают, что «строже» не всегда значит «лучше» — на этом список и учит.
FILTER_CHOICES = [

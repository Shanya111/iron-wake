...
#
# ЗАМЕР 29 августа 2026 (filters_new.py в лаборатории) — ПЕРЕСЧИТАН ЗАНОВО на движке
...
FILTER_CHOICES = [
    (0.0,   0.0, "выкл",              466, -0.050, -863.5),
    (0.075, 0.0, "вход 0.075",         83, -0.040, -171.1),
    (0.05,  0.0, "вход 0.05",          55, -0.029,  -76.4),
    (0.075, 1.0, "вход 0.075+риск 1",  38, -0.017, -103.3),
]
...
def settings_text(user_id: int, is_admin: bool) -> str:
    ...
    return (
        ...
        "⚠️ НО ПОСЛЕ ИЗДЕРЖЕК выигрывает «выкл», и заметно. Фильтры отбирают сделки у "
        "самого уровня, а у них узкий стоп; комиссия берётся в процентах цены и делится "
        "на риск, поэтому сделка дорожает ровно тогда же, когда растёт качество отбора. "
        "Включать фильтр стоит, если ты веришь, что исполняешь лучше модели.\n\n"

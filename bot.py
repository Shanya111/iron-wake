...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "BingX (14 крипто-пар, золото, нефть, 5 валютных пар). Разбираю расклад по VSA "
...
def _format_engine_view(info: dict, ex: dict, zones: list[dict], ob: dict | None,
    ...
    """Отчёт /analyze: те же шесть условий, по которым движок принимает решение.
    ...
    # Объём мерится на СИГНАЛЬНОЙ свече — той, по которой движок принимает решение.
    # Но сам прокол мог случиться раньше и вернуться за уровень только сейчас
    # (config.SWEEP_WAIT_BARS): тогда человеку надо сказать, КОГДА сняли ликвидность,
    # иначе он будет искать прокол на последней свече и не найдёт.
    # Число печатаем по сторонам: у лонга и шорта свип бывает на разных свечах.
    lines += ["", f"3. Объём сигнальной свечи — той, что вернула цену за уровень "
...
def _analysis_prompt(info: dict, ex: dict, zones: list[dict],
    ...
    out = [
        ...
        # Формулировка обязана совпадать с пунктом 3 отчёта: там человек читает
        # «объём СИГНАЛЬНОЙ свечи», и если тут написать иначе, ИИ-комментарий под
        # отчётом объяснит правило, которого у движка нет.
        f"Объём сигнальной свечи — той, что вернула цену за уровень: "
        f"{ex['vol_ratio']:.1f}× среднего, порог ×{ex['vol_mult']:g}",
    ]
    if ex.get("pools"):
        near_pools = sorted(ex["pools"], key=lambda x: abs(x - c))[:3]
    ...
    for side in live:
        ...
        out.append(f"{side}: отбой " + ("не измерить (свеча без размаха)" if reb is None
                                             ...
                                             f"порог {config.MIN_CLOSE_POS:g}"))
    for side in live:
        s = ex["sides"][side]
        if signals.get(side):
...
# Поля: (MAX_ENTRY_DIST_ATR, MAX_RISK_ATR, подпись, сигналов в месяц, БРУТТО R на сделку,
...
#
# ЗАМЕР 2 сентября 2026 (filters_new.py в лаборатории) — ПЕРЕСЧИТАН ЗАНОВО на движке
# ветки: фильтра R:R нет, пороги в долях ATR, стоп за дальним из двух экстремумов + 0.25
# ATR, вход по закрытию. Прежние подписи (88/мес, −0.044R) измерены на движке С фильтром
# R:R и здесь не действуют вообще. Якорь прогона воспроизвёл боевую конфигурацию main
# (2397 сигналов, −0.044 R), поэтому строкам можно верить.
#
# СЕТКА ПОРОГОВ РИСКА ДРУГАЯ, и это не произвол: стоп стал вдвое шире, типичный риск
# сделки теперь 0.6–1.8 ATR вместо прежних долей. Пороги 0.5 и 0.75 на нём резали бы
# почти всё, и сравнение выродилось бы в сравнение пустых выборок.
#
# КАК ОТОБРАНЫ ЭТИ ЧЕТЫРЕ. Критерий тот же, что 20 августа: в списке только
# НЕДОМИНИРУЕМЫЕ строки. Вариант доминируем, если другой даёт И больше сигналов, И
# лучшую среднюю сделку одновременно — такой не нужен никогда. Из шестнадцати
# прогнанных строк доминируемыми оказались двенадцать, включая все пороги «вдогонку»
# поодиночке (риск 0.75 / 1 / 1.25 / 1.5 / 2 — все хуже, чем «выкл»).
#
# Оставшиеся четыре образуют лестницу: сверху вниз сигналов меньше, средняя сделка
# лучше, монотонно и без исключений (466 → 83 → 55 → 38 сигналов при
# −0.050 → −0.040 → −0.029 → −0.017 R брутто).
#
# И ГЛАВНАЯ ОГОВОРКА, без которой подписи врут. На кнопке БРУТТО — качество отбора до
# издержек. ПОСЛЕ издержек лучшая строка — «выкл», и с большим отрывом: нетто −0.136
# против −0.162 у «вход 0.075», −0.151 у «вход 0.05» и −0.207 у сочетания. Механизм
# известен и воспроизводится в седьмой раз: фильтры отбирают сделки у самого уровня, а
# у них узкий риск; издержки берутся в процентах цены и делятся на риск, поэтому цена
# сделки в R растёт ровно тогда же, когда растёт качество отбора (0.086 → 0.12–0.19 R).
# Полная таблица — в CLAUDE.md, «Кнопки /settings пересчитаны».
FILTER_CHOICES = [
    (0.0,  0.0,  "выкл",       302, -0.030, -697.1),
    (0.25, 0.0,  "вход 0.25",  101, -0.023, -234.5),
    (0.0,  1.25, "риск 1.25",   50, +0.003, -132.1),
    (0.0,  1.0,  "риск 1",      18, +0.048,  -32.8),
]
...
def settings_text(user_id: int, is_admin: bool) -> str:
    ...
    return (
        ...
        "Фильтры действуют на все 21 инструмент движка одинаково, включая валютные "
        "пары: отдельных порогов у форекса нет.\n\n"
        "На кнопке — что ты получишь по замеру за 833 дня на 14 инструментах BingX: "
        "сколько сигналов в месяц и какая выходит средняя сделка ДО издержек (в R). "
        "Работает ровно ОДНА кнопка: нажал новую — прежняя выключилась.\n"
        "Кнопки идут лестницей: ниже по списку — реже сигналы, но лучше средняя сделка. "
        "Варианта, который выигрывал бы сразу по обоим столбцам, в списке нет.\n"
        "После издержек разница между кнопками невелика и все они в минусе: от −0.066 R "
        "на сделку у «риск 1» до −0.096 у «риск 1.25». Фильтры отбирают сделки у самого "
        "уровня, а у них узкий стоп; комиссия берётся в процентах цены и делится на "
        "риск, поэтому сделка дорожает ровно тогда же, когда растёт качество отбора — "
        "и почти съедает выигрыш от отбора.\n\n"
...
def settings_keyboard(user_id: int, is_admin: bool) -> InlineKeyboardMarkup:
    ...
    for dist, risk, label, per_month, r_trade, _fresh in FILTER_CHOICES:
        ...
        mark = "✅ " if active else ""
        rows.append([InlineKeyboardButton(
            text=f"{mark}{label} · {per_month}/мес · {r_trade:+.2f}R",
...
async def _nl_subscribe(message: Message, intent: dict, action: str) -> None:
    ...
    if code not in engine_codes():
        await message.answer("Подписка на сигналы — по инструментам движка: крипта, "
                             "золото, нефть и валютные пары (фьючерсы BingX). "

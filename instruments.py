...
В реестре 21 инструмент, В ДВИЖКЕ 16 (14 крипты + золото + нефть): пять валютных пар
остаются в реестре ради алертов и журнала, но сигналов по ним нет — см. engine_codes().
...
#
# ФОРЕКС ИЗ ДВИЖКА УБРАН 29 августа 2026 (решение владельца: движок только по крипте
# и товарам). Он побывал в движке с 21 августа — на синтетических фьючерсах BingX
# NCFX*, со своими порогами детектора в долях ATR. Причина вывода — деньги: замер
# 20 августа дал -1.385 R на сделку, из которых 1.153 R это ИЗДЕРЖКИ. Механизм —
# у форекса риск сделки в процентах цены мал (0.09% против 0.50% у крипты), а
# комиссия в процентах цены та же, поэтому та же сделка стоит в R на порядок дороже.
# Ни одна из 21 проверенной конфигурации не вышла в плюс. См. CLAUDE.md, «Попытка
# вылечить форекс».
#
# ПАРЫ ОСТАЛИСЬ В РЕЕСТРЕ и с полем "ccxt" — под алерты, журнал сделок и NL-роутер:
# «поставь алерт EURUSD 1.17» и «взял EURUSD по …» обязаны работать. Из движка их
# выводит engine_codes() по флагу "fx", а не отсутствие источника данных. Убрать
# источник было бы проще, но тогда сломались бы и алерты, и журнал, чего никто
# не просил.
#
# Отдельных порогов детектора у них больше нет: с 29 августа ВСЕ пороги заданы в
# долях ATR (config.BREAK_ATR / STOP_ATR), и держать вторую мерку стало незачем.
#
...
INSTRUMENTS = {
    # ── Форекс: синтетические бессрочные фьючерсы BingX (NCFX*). Флаг "fx" ВЫВОДИТ
    #    пару из движка (см. engine_codes) — сигналов по ней нет, а алерты и журнал
    #    работают как раньше.
...
def engine_codes() -> list[str]:
    ...
    Сейчас это 16 бессрочных фьючерсов BingX: 14 крипты + золото + нефть. Своя пара
    в движок не входит.

    Условий два, и второе появилось 29 августа 2026. Источник объёма (data_source) —
    без него движку нечего считать. И НЕ валютная пара: форекс из движка выведен по
    деньгам, хотя данные у него полноценные (см. шапку файла). Признак — флаг "fx",
    а не отсутствие источника: источник им нужен для алертов и журнала."""
    return [code for code in INSTRUMENTS if data_source(code) and not is_fx(code)]
...
def is_fx(code: str | None) -> bool:
    """Валютная ли это пара — то есть НЕ входит ли она в движок.

    До 29 августа 2026 флаг значил другое: «у пары свои пороги детектора». Теперь
    пороги у всех одни (в долях ATR), и единственный смысл флага — вывести пару из
    engine_codes(): анализа и сигналов по форексу нет. Алерты, журнал сделок и
    NL-роутер по этим парам работают как работали. Почему выведены — в шапке файла.
    """

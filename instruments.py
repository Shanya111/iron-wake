...
def tick_size(pair: str, price: float | None = None) -> float:
    """Шаг цены инструмента — по точности отображения из реестра.

    Нужен стопу ложного пробоя: с 17 сентября 2026 он стоит за фитилём свечи плюс
    config.JUNE_STOP_TICKS тиков.

    ОГОВОРКА, КОТОРУЮ ВАЖНО ПОМНИТЬ: `decimals` в реестре выбраны для ПОКАЗА человеку,
    а не взяты с биржи. У BTC это 2 знака (тик 0.01), у AVAX тоже 2 — и в долях ATR
    выходит совсем разное: 0.0001 ATR против 0.61 ATR. Настоящие шаги цены контрактов
    BingX лежат в ccxt (markets[symbol]["precision"]) и в реестр не заводились.

    Своя пара (символ контракта вне реестра) — точность подбираем по цене, как и везде.
    """
    d = resolve(pair)["decimals"]
    if d is None:
        d = infer_decimals(price) if price else 4
    return 10.0 ** -int(d)

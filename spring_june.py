...
менялся. Вернул их в бой владелец 15 сентября 2026 (config.SPRING_ENGINE = "june23"):
  ...
  • свеча проколола уровень глубже BREAK_PCT (0.05%) цены уровня и закрылась обратно
    за него. Из проколотых берётся САМЫЙ ГЛУБОКИЙ (в июне брался первый подошедший по
    списку). Сила уровня отбор НЕ решает: требование «только сильные» прожило с 17 по
    22.09.2026 и выключено (config.JUNE_STRONG_ONLY) — оно стоило 98.6% сигналов;

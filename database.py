    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS ict_signals (
                ...
                status      TEXT    NOT NULL DEFAULT 'open',  -- open | hit_tp | hit_sl | expired | manual
                result_r    REAL,
                created_at  TEXT    NOT NULL,
                closed_at   TEXT,
                UNIQUE(instrument, direction, bar_time)
        ...
        """)
        # Сигналы, выданные на ПЯТНАДЦАТИМИНУТКАХ, снимаются с ведения (19.09.2026,
        # решение владельца вместе с переносом стратегии на часовик). Причина не в
        # аккуратности: у них другая геометрия — стоп считался по M15 и в среднем вдвое
        # уже часового, — а исход теперь считался бы по часовым свечам. Вести сделку по
        # правилам, по которым её не открывали, нельзя.
        #
        # Статус 'manual' — тот же, что у позиций тренда, закрытых рукой владельца.
        # Условий самоограничения ДВА, и нужны оба: минуты свечи не ноль (часовой сигнал
        # стоит на круглом часе и не попадает) И запись создана до переноса — иначе при
        # возврате на M15 миграция снимала бы свежие сигналы при каждом рестарте. Тот же
        # приём, что у миграции доавгустовских алертов.
        conn.execute("""
            UPDATE ict_signals SET status = 'manual', closed_at = ?
            WHERE status = 'open' AND created_at < '2026-09-20'
              AND substr(bar_time, 15, 2) <> '00'

...
## ГЛАВНОЕ: деплой запускается САМ по пушу в `main`

На сервере стоит вебхук-автодеплой — `/opt/projects/web/deploy/hooks.json` плюс
`deploy-bot.sh`. **Пуш в `main` на GitHub запускает всю процедуру целиком**, без единой
команды с твоей стороны:

1. `git reset --hard origin/main` в `/opt/apps/bot`;
2. бэкап базы в `bot.db.predeploy-ГГГГММДД-ЧЧММСС` (делает сам скрипт, строка 64–65);
3. `docker build` образа `iron-wake-bot:local`;
4. `docker compose up -d bot`;
5. проверка здоровья: `Running`, `RestartCount`, `getMe` — и запись в
   `/opt/projects/web/deploy/deploy.log` вида `health OK -> USPEH <sha>`.

Занимает это около 20 секунд. Проверено 19 августа 2026: пуш в 19:05:0x → образ собран
19:05:32 → контейнер поднят 19:05:34 → `getMe HTTP=200` в 19:05:51.

**Что из этого следует.** Ручная процедура ниже нужна ТОЛЬКО когда автодеплой не сработал
или ты хочешь выкатить код, которого нет в `origin/main`. В обычном релизе твоя работа —
запушить и **проверить журнал**:

```bash
ssh main 'tail -20 /opt/projects/web/deploy/deploy.log'   # ищи "health OK -> USPEH <sha>"
```

Если пойти по ручной инструкции не глядя, легко начать деплоить поверх уже выкаченного —
и, что хуже, сделать «бэкап перед миграцией» уже ПОСЛЕ неё. Настоящая точка отката базы —
та, что создал сам автодеплой (`bot.db.predeploy-*` с меткой времени релиза).

> Watchdog (`watch/watchdog.sh`, cron каждые 15 минут) — это НЕ деплой, а сторож: он
> проверяет здоровье и шлёт алерты. Код он не выкатывает.
...
> **УСТАРЕЛО (проверено 19 августа 2026).** Сейчас в `Dockerfile.bot` стоит
> `COPY *.py ./` — то есть новые `.py`-файлы попадают в образ САМИ, править Dockerfile
> при добавлении модуля больше не нужно. Раздел оставлен на случай, если копирование
> когда-нибудь вернут к поимённому. Проверить: `ssh main 'grep -n "^COPY"
> /opt/projects/web/Dockerfile.bot'`.
...
В Telegram: `/start` отвечает, `/analyze` показывает инструменты.

...
не там, где стоит, а хуже, иногда сильно. По крипте рынок работает, но на
выходных ликвидность тонкая, а движения рваные, и та же турбулентность бьёт по
позиции, за которой некому следить.

Отсюда два правила (оба настраиваются в config):
  • новых входов не даём в дни NO_ENTRY_WEEKDAYS (пятница, суббота, воскресенье)
...
def is_entry_allowed(ts) -> tuple[bool, str]:

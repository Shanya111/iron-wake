...
def tf_candles(h1: pd.DataFrame, hours: int) -> pd.DataFrame:
    """Часовые свечи, укрупнённые до `hours`-часовых по календарной сетке UTC.

    Последняя свеча может быть НЕПОЛНОЙ, и это не небрежность: боевой дневной
    фильтр направления точно так же видел сегодняшний день недоделанным, и замер,
    по которому выбрано шестичасовое окно, считался именно так.

    Пустые корзины выбрасываются — у валютных пар вне сессии в сетке бывают дыры.
    """
    if h1.empty:
        return h1
    out = h1.resample(f"{hours}h").agg(
        {"open": "first", "high": "max", "low": "min", "close": "last", "volume": "sum"})
    return out.dropna(subset=["close"])


def engine_trend(h1: pd.DataFrame) -> str:
    """Направление для движка ложного пробоя: EMA20 по свечам `TREND_TF_HOURS`.

    Правило то же, что у get_trend (цена против EMA20 плюс наклон), меняется
    только таймфрейм, на котором оно считается. Замер и цена решения — в
    комментарии к config.TREND_TF_HOURS.

    Считаем по ЗАКРЫТЫМ часам: последняя строка `h1` — формирующийся час (так
    отдаёт биржа), и её надо отбросить. Иначе направление дёргалось бы внутри
    часа, а решение движок принимает по закрытой свече (df.iloc[-2]).

    Живёт здесь, а не в scheduler, ровно по той же причине, что explain лежит
    рядом с детектором: сигналы и разбор /analyze обязаны спрашивать направление
    одной и той же функцией, иначе отчёт начнёт обещать не то, что придёт.
    """
    closed = h1.iloc[:-1] if len(h1) > 1 else h1
    return get_trend(tf_candles(closed, config.TREND_TF_HOURS))

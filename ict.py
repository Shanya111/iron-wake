"""ICT — свип ликвидности и разрыв (FVG) на часовых свечах. Стратегия №6.
...
НА ЧАСОВИКЕ С 19.09.2026. Первый день стратегия работала на M15; правила при переносе
не изменились ни на строку — поменялись ICT_TIMEFRAME и ICT_TF_MINUTES. Детектор
считает по БАРАМ, а часы из констант (срок жизни пула, горизонт) пересчитываются в
бары в одном месте, _pool_table. Обратно на M15 — теми же двумя числами.

ПРАВИЛА — ровно тот вариант, что мерился (`ict.py` в лаборатории, строка «вход по
рынку, цель >= 1R»); для лонга, шорт зеркально:
...
в лаборатории; 14 инструментов, H1 за 833 дня и M15 за 100 дней):
  • на часовике брутто +0.019 R на сделку, нетто −0.031 R — лучше M15 по всем
    колонкам, но экзамен вне выборки отрицательный (+0.033 до 01.02.2026, −0.017
    после), и прибыльной стратегия не является;
  • M15 стоил 0.125 R за сделку против 0.050 R на часовике: там стоп узкий
    (1.03% цены против 2.72%), а комиссия берётся в процентах цены — девятое подряд
    воспроизведение закона «издержки в R», здесь в нашу пользу;
...
ЧАСТОТА. 611 событий в месяц на 14 инструментов после дедупа 90 минут — около
1.5 в день на инструмент (на M15 было 2480 и шесть). Крутится ICT_DEDUP_MIN.

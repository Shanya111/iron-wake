...
async def get_candles(
    ...
    ВНИМАНИЕ: ключ кеша — (биржа, символ, таймфрейм), БЕЗ limit. Кто запросил первым
    после истечения TTL, тот и задал глубину для всех остальных в этом цикле. Поэтому
    все задачи планировщика просят H1 одной глубиной (config.H1_FETCH_LIMIT): если бы
    трекер успел запросить 120 свечей раньше монитора, тому не хватило бы истории на
    ресемпл H4, и слой H4 молча оказался бы на 30 барах вместо 120.
    """

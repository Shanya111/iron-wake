...
- **Торговый ассистент (VSA + Spring)** по крипте и форексу (через Kraken — там настоящий объём)
  плюс золоту и нефти (через Yahoo — частичный объём, без стакана): контекстный анализ (тренд D1 +
...
- Проверяется последняя **закрытая** H1-свеча (`df.iloc[-2]`). Фильтр тренда: бычий сигнал не
  ...
  ± `STOP_SPREAD`. Приоритет `high`, если пробитый уровень `strong`.
...
| Команда | Что делает |
|---|---|
| `/analyze [код]` | Анализ инструмента: тренд D1 + сильные/слабые уровни + зоны ликвидности + стакан (DOM: давление/спред/стены — **только у биржевых инструментов**; у золота/нефти стакана нет, блок пропускается). Поверх чисел — гибридный AI-разбор через OpenRouter (ошибка LLM не критична). |

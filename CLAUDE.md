...
- **Торговый ассистент (VSA + Spring)** по 21 бессрочному фьючерсу **BingX** (14 крипты +
  ...
  **СВИП ОБЯЗАН БЫТЬ СВИПОМ** (5 сентября 2026): движок больше не путает снятие
  ликвидности с выкупом уровня снизу, свипает ПУЛЫ равных экстремумов наравне с
  уровнями-фракталами, и разрешает свипу и выкупу быть РАЗНЫМИ свечами — см.
  «Свип обязан быть свипом».
...
- **Свип и выкуп могут быть РАЗНЫМИ свечами** (`SWEEP_WINDOW` 1): свеча А уходит за
  уровень на объёме, свеча B ПОГЛОЩАЕТ её тело и закрывается обратно. Объём мерится
  на свече свипа (`_sweep_bars`), сила отбоя — на свече выкупа. Сигнал по закрытию B.
...
#### 3. Свип и выкуп — разные свечи (`SWEEP_WINDOW` = 1)
...
### Тесты
`python tests/test_engine.py` — 88 юнит-тестов аналитики, паттернов, ATR и двух фильтров
...
ликвидности**, **двухсвечного свипа**, **сборки скользящего часа** и

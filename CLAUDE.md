...
- **Торговый ассистент (VSA + Spring)** по 16 бессрочным фьючерсам **BingX** (14 крипты +
  ...
  **Форекса в движке нет** — он неработоспособен в этой стратегии и на BingX отсутствует;
  простые алерты «касание уровня» по валютным парам работают как раньше
...
- yfinance + pandas — котировки и минутные свечи по любому тикеру Yahoo Finance (простые алерты).
  Часовые/дневные OHLCV Yahoo (`get_hourly_candles`/`get_daily_candles`) остались для журнала
  сделок по своим тикерам; движок ими больше не пользуется
...
│                         get_hourly_candles/get_daily_candles (Yahoo-свечи для журнала сделок)
...
├── pattern_detector.py — детектор Spring (лонг) и Upthrust (шорт), цена лимитной заявки
│                         (limit_price), исполнение заявки (evaluate_fill), исход (evaluate_signal)
...
четыре валютных (EUR/USD, GBP/USD, AUD/USD, USD/CAD — **только простые алерты**),
...
- `None` — форекс, своя пара и прочее без источника: только простые алерты «касание уровня».

Простые алерты «касание уровня» работают по всем парам через yfinance (`ticker`) независимо
от движка — в том числе по валютным.
...
`ccxt` убрано — они остались в реестре **только для простых алертов**.
...
Поверх простых алертов работает торговый ассистент по 16 **бессрочным фьючерсам BingX**:
...
«касание уровня» при этом не тронуты — движок живёт отдельными модулями и таблицами, и по валютным
парам алерты работают как раньше через yfinance.
...
| Команда | Что делает |
|---|---|
| `/analyze [код]` | Анализ инструмента: тренд D1 + сильные/слабые уровни + зоны ликвидности + стакан (DOM: давление/спред/стены — теперь **у всех 16 инструментов движка**, включая золото и нефть: на BingX у них есть стакан, которого не было на Yahoo). Поверх чисел — гибридный AI-разбор через OpenRouter (ошибка LLM не критична). |
...
- **Действующие пороги** = `config.effective(database.get_user_settings(user_id))` — личные
  ...
  его в `analyzer.find_liquidity_zones(d1, mult)`. В колбэке id берётся из `call.from_user`
...
### Тесты
`python tests/test_engine.py` — 34 юнит-теста аналитики, паттернов, лимитного входа, трекинга
исхода и стакана на синтетических данных (сети не требуют, запускаются и под pytest).

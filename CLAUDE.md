...
## Что делает
- **ЛОЖНЫЙ ПРОБОЙ СНЯТ С БОЯ** (15 сентября 2026, решение владельца,
  `config.SPRING_SIGNALS = False`): бот не ищет и не рассылает сигналы Spring/Upthrust,
  сигналы присылает только стратегия №4. Код движка, `/analyze`, тесты и замеры на месте —
  константа включает обратно; открытые сигналы доводятся до исхода. Вариант на замену
  лежит в ветке `rr-filter-and-m15-early` и не выбран — см. «Ложный пробой выключен»
...
- `/settings` крутит **строгость отбора** (два ATR-фильтра) — см. «Строгость отбора»
  Замер этих кнопок пересчитан 2 сентября: на движке без фильтра R:R прежние подписи
  не действуют
...
- `/analyze` — разбор по шести условиям остаётся справкой, но в итоге пишет, что
  ...
  пообещает сигнал;
- `/settings` — фильтры на месте, меню помечает, что влияют они только на разбор;
...
Шесть задач, **с 15 сентября 2026 в бою четыре**: `run_analysis` и `monitor_signals` не
ставятся при `SPRING_SIGNALS = False` (см. «Ложный пробой выключен»).
...
| `/subscribe` | Вкл/выкл подписку на сигналы по инструменту (кнопки по всем 21 инструменту движка, галочка = подписан). Валютные пары в списке ЕСТЬ — с 3 сентября 2026 они снова в движке. |
...
| `/settings` | **Строгость отбора сигналов** — действует на все 21 инструмент движка одинаково, включая валютные пары: два ATR-фильтра (`MAX_ENTRY_DIST_ATR` — «вход у уровня», `MAX_RISK_ATR` — «не входить вдогонку»), каждый включается независимо, `0` = выключен. Подписчик крутит свои значения (таблица `user_settings`), админ — общий дефолт (`settings.json`). В тексте меню сразу показан замер и честная оговорка, что прибыльным движок это не делает. См. «Строгость отбора». |
...
### Тесты
`python tests/test_engine.py` — 102 юнит-теста аналитики, паттернов, ATR и двух фильтров

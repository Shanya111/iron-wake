...
- **ЛОЖНЫЙ ПРОБОЙ В БОЮ ПО ПРАВИЛАМ 23 ИЮНЯ** (15 сентября 2026, решение владельца,
  ...
  (17 сентября), уровень обязан быть СИЛЬНЫМ, а среднее объёма считается за 12 часов
  вместо 20 (17 сентября, см. «Только сильные уровни и окно объёма 12» — там же цена:
  сигналов 469 в месяц → 16). **Всё, что ниже написано про правила
...
- **СТРАТЕГИЯ №6 — ICT НА 15-МИНУТКАХ, В БОЮ С 18 сентября 2026** (решение владельца,
  ...
  встречная ликвидность не ближе 1 риска. **Цена решения названа до включения:** брутто
  −0.035 R, нетто −0.160 R на сделку, а сам свип ликвидности в замере НЕОТЛИЧИМ от
  случайной линии (25959 событий против 30683 плацебо, разница −0.006 ±0.023). Бот
  сверен с замером бар в бар: BTC 88/88, GOLD 72/72 — см. «Стратегия №6»
...
- APScheduler (`AsyncIOScheduler`) — фоновые задачи движка (планировщик один, в `scheduler.py`):
  ...
  тренд по недельному каналу (5 мин), пробой сильного уровня (5 мин, в слежке),
...
├── breakout.py         — стратегия №5, пробой сильного уровня (в слежке, сигналы не
│                         рассылаются): detect / outcome / result_r, чистые функции
├── ict.py              — стратегия №6, ICT на M15 (в бою с 18.09.2026, вместо тренда):
...
- **три теста** (122 → 125): слабый уровень сигнала не даёт ни в лонг, ни в шорт;
  ...
  повыше; `explain` называет именно эту причину. Все три падают при
  `JUNE_STRONG_ONLY = False` — проверено прогоном.
...
### Стратегия №6 — ICT: свип ликвидности и разрыв на M15 (18 сентября 2026)
...
|---|---|---|---|---|---|---|---|---|
| **M15, вход по рынку, цель ≥1R (В БОЮ)** | 7731 | 2480 | 45% | 1.16 | **−0.035** | 0.125 | **−0.160** | 0% |
| M15, канон школы (лимитка в разрыв) | 379 | 122 | 48% | 0.63 | −0.274 | 0.188 | −0.462 | 95% |
| H1, та же модель по рынку, цель ≥1R | 16628 | 611 | 46% | 1.29 | +0.019 | 0.050 | −0.031 | 0% |
| H1, канон школы (лимитка в разрыв) | 1222 | 45 | 58% | 0.76 | −0.078 | 0.056 | −0.134 | 93% |

**ЦЕНА РЕШЕНИЯ НАЗВАНА ДО ВКЛЮЧЕНИЯ, и она тройная:**

1. **Прибыльной стратегия не является.** Брутто −0.035 R, нетто −0.160 R на сделку.
2. **M15 дороже H1 ВТРОЕ**: 0.125 R за сделку против 0.056 R, потому что стоп узкий
...
**ЧАСТОТА — главная практическая оговорка.** 2480 событий в месяц на 14 инструментов
после дедупа, то есть около 6 в день на инструмент; на живых данных вышло 20–29 сигналов
за 75 часов на инструмент. Подписка на пять монет — это примерно тридцать сообщений в
сутки. Крутится одной константой `ICT_DEDUP_MIN`.
...
`/breakout` (команда только в админском меню).

**СИГНАЛОВ НИКОМУ НЕ ШЛЁТ** (`config.BREAKOUT_SIGNALS = False`, решение владельца):
бот считает их и доводит до цели/стопа/истечения, а владелец смотрит месяц на живых
данных и решает. `True` включает рассылку подписчикам инструмента.
...
- `monitor_ict(bot)` — каждые 5 минут: стратегия №6, ICT на пятнадцатиминутках. Ищет
...
- `monitor_breakout(bot)` — каждые 5 минут: стратегия №5, пробой сильного уровня.
  Считает сигналы по всем инструментам движка и доводит их до исхода, но НИКОМУ НЕ
  ШЛЁТ, пока `config.BREAKOUT_SIGNALS = False`. См. «Стратегия №5».
...
| `/ict` | **ICT — свип ликвидности и разрыв на M15** (стратегия №6, в бою с 18.09.2026 вместо тренда): открытые сигналы и закрытые сделки с итогом в R. Сигналы приходят подписчикам инструмента по стратегии ICT (💧 в `/subscribe`). |
...
| `/breakout` | **Пробой сильного уровня** (стратегия №5, в слежке): сколько сигналов насчитано, открытые и закрытые, итог в R. Сигналы никому не рассылаются — см. «Стратегия №5». Команда только в админском меню. |
...
| user_id | INTEGER | Telegram ID |
| instrument | TEXT | Код инструмента |
| strategy | TEXT | Код из `config.STRATEGIES`: `spring` / `ict` (до 18.09.2026 — `trend`, подписки мигрировали) |
| created_at | TEXT | ISO 8601 |
...
| bar_time | TEXT | Свеча решения (M15, UTC) — якорь трекинга исхода |
...
### Тесты
`python tests/test_engine.py` — 146 юнит-тестов (с 18.09.2026 в том числе стратегия №6 ICT:

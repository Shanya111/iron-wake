...
- **Стратегия №4 — ТРЕНД ПО НЕДЕЛЬНОМУ КАНАЛУ** (14 сентября 2026, `trend.py`): вторая
...
### Тесты
`python tests/test_engine.py` — 113 юнит-тестов (с 15.09.2026 в том числе ложный пробой по
...
**Девять тестов стратегии №4** (14 сентября 2026, `test_trend_*`) — список в разделе
...
| **край диапазона против середины** (пункт 4 картинок) | средне | **ни разу** |
...
3. **Свип на краю диапазона, а не в середине** — прямая проверка утверждения картинки 4.
   Нужно определение диапазона (например, положение свипа внутри окна N свечей).
   **Остался единственным неизмеренным пунктом схемы, который дёшев.**

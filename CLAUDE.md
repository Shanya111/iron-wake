...
5. **R:R при входе прямо сейчас** — с риском в ATR и с целью. С 2 сентября 2026 это
...
**Замер пересчитан 2 сентября 2026** (`filters_new.py`), потому что прежние подписи мерились
...
|---|---|---|
| **displacement** (сила импульса возврата, а не факт закрытия за уровнем) | да, H1 есть | частично: «отбой» и «поглощение» — грубые однобарные версии, ОБЕ СРАБОТАЛИ |
| **BOS / CHoCH** (пробит ли предыдущий свинговый экстремум после свипа) | да, фракталы уже считаются | **ни разу** |
...
1. **Displacement по-настоящему** — не «где закрылась свеча», а размер импульса возврата:
   от экстремума свипа цена должна пройти N × ATR за 1–3 часа. Прямое продолжение
   единственного сработавшего рычага.
2. **BOS после свипа** — пробила ли цена предыдущий свинговый максимум (для лонга).
   Фракталы уже есть в `_levels_at`, мерить дёшево, не мерилось никогда. Самая
   отличительная недостающая деталь схемы.
...
4. **FVG как зона входа** — самое дорогое и самое рискованное: отложенный вход подводит
   цену ближе к стопу, риск сужается, издержки дорожают. Браться только если 1–3 дали
   результат.

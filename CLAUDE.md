...
- **Торговый ассистент (VSA + Spring)** по 16 бессрочным фьючерсам **BingX** (14 крипты +
  ...
  **Форекс из движка выведен** 2 сентября: пары остались в реестре ради алертов, журнала и
  NL-роутера, но сигналов по ним нет — см. «Форекс выведен из движка».
  **СКОЛЬЗЯЩИЙ ЧАС** (3 сентября 2026): свеча по-прежнему часовая, но собирается заново
  каждые 15 минут — сигнал больше не ждёт круглого часа, задержка упала с часа до
  четверти. Правила детектора при этом не изменились ни одним порогом — см.
  «Замер 3 сентября 2026 — 15-МИНУТКИ»
...
14 крипто-пар. **В ДВИЖКЕ из них 16** — валютные пары выведены 2 сентября 2026
...
владельца («максимум сигналов»), зная, что экономика отрицательная. Вывели 2 сентября —
...
- Проверяется последняя **закрытая** часовая свеча (`df.iloc[-2]`). Фильтр тренда: бычий
  ...
  `strong`.
- **СКОЛЬЗЯЩИЙ ЧАС** (`config.ROLLING_HOUR`, 3 сентября 2026): свеча остаётся часовой, но
  собирается заново каждые 15 минут из пятнадцатиминуток — час кончается на последней
  закрытой M15, а не на круглом часе. Правила детектора не меняются ни одним порогом;
  меняется МОМЕНТ, в который свечу можно считать закрытой, и задержка сигнала падает с
  часа до пятнадцати минут. Собирает `analyzer.rolling_hours(m15, limit)`, отдаёт движку
  `scheduler.engine_candles(code)`. Соседние часы НЕ перекрываются (шаг ровно 4 бара) —
  иначе объём и ATR считались бы по размазанному среднему и все пороги поехали бы.
  Замер и цена решения — в разделе «Замер 3 сентября 2026 — 15-МИНУТКИ».
...
### Форекс выведен из движка (2 сентября 2026)

Валютные пары пробыли в движке восемь дней — с 21 по 2 сентября. Выведены по деньгам,
...
парам пороги в долях ATR, пока у всех остальных пороги были в процентах цены. С 2 сентября
...
| Команда | Что делает |
|---|---|
| `/analyze [код]` | **Расклад глазами движка**, а не описание рынка вообще: те же шесть условий и в том же порядке, в каком их проверяет детектор. См. «Команда /analyze». Плюс зоны ликвидности и стакан (DOM — у всех 16 инструментов движка) и AI-разбор поверх (ошибка LLM не критична). |
...
### Тесты
`python tests/test_engine.py` — 79 юнит-тестов аналитики, паттернов, ATR и двух фильтров
строгости, лимитного входа, трекинга исхода, стакана, разбора `explain()`, **порогов в
долях ATR**, **силы отбоя**, **минимальной цели**, **сборки скользящего часа** и
...
| **F2. скользящий час, уровни с календарного** ← **вшито** | 1572 | 570 | 58% | 0.97 | 1.32% | **+0.011** | 0.089 | **−0.078** | −122.2 |

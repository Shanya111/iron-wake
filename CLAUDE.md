...
`BREAK_ATR` 0.05 и запас стопа `STOP_ATR` 0.10. Отдельной мерки для отдельных
...
- **Пороги — в долях ATR, а не в процентах цены** (29 августа 2026). «0.05% цены» у биткоина
  ...
  Отсюда `BREAK_ATR` 0.05 и `STOP_ATR` 0.10 — **ровно медиана набора**: «в среднем по движку»
  отбор остался прежним, а перекос между инструментами ушёл. Значения не назначены, а взяты
  по эквиваленту — это важно, потому что позволяет сравнивать новые числа со старыми.

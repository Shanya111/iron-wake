...
│                         CRUD уровней/сигналов/подписок движка, журнал сделок, get_hourly_candles
...
- `track_trades(bot)` — каждые 5 минут: исход открытых сделок журнала (таблица `trades`). Свечи:
  для инструментов движка — Kraken H1 (общий кеш), для золота/нефти/своей пары — часовые свечи
  Yahoo. `expired` трактуется как «ещё открыта» (журнал не истекает).

...
- **ЛОЖНЫЙ ПРОБОЙ В БОЮ ПО ПРАВИЛАМ 23 ИЮНЯ** (15 сентября 2026, решение владельца,
  ...
  движок в `pattern_detector.py`: он в коде, но сигналы шлёт не он.** Какие сигналы
  присылать, подписчик выбирает **галочками стратегий в `/subscribe`** — см. «Ложный
  пробой: правила 23 июня и галочки стратегий»
...
**Галочки стратегий в `/subscribe`.** Сверху две кнопки — «Ложный пробой» и «Тренд по
недельному каналу» (`config.STRATEGIES`), ниже инструменты. Сигнал приходит, если отмечены
и стратегия, и инструмент (`database.get_subscribers(instrument, strategy)`). В базе
хранится ОТКАЗ (таблица `strategy_off`), поэтому **по умолчанию у всех включены обе** — уже
подписанные получают сигналы обеих стратегий без миграции. Исход уже отправленного сигнала
ложного пробоя уходит его владельцу и после снятия галочки; сообщения тренда о стопе и
выходе — нет, они идут текущим подписчикам стратегии.

Тесты (`test_june_*`, `test_spring_rules_follow_engine_switch`,
`test_strategy_checkbox_filters_subscribers`): июньский детектор берёт то, что сентябрьский
бракует (вялый отбой, выкуп уровня снизу), прокол мерится долей цены, стоп и цель как в
июне, `explain` согласен с детектором, галочка фильтрует рассылку своей стратегии и не
трогает другую.
...
ETH и SOL. На валюте и нефти стратегия на истории теряла, на BTC и золоте давала ноль. Чтобы
это было видно до входа, сообщение о входе печатает строку «Замер на истории по X» из
`config.TREND_MEASURED`. **Получатели — подписчики инструмента** (`/subscribe`), те же, кто
...
- сообщение о входе: цена ≈ открытие часа, стоп и его расстояние в %, текущая граница
  выхода, объём под риск 1% депозита (с плечом, если объём больше депозита) и замер по
  инструменту; о стопе и выходе — итог в R. Запоздало больше чем на 2 часа — пометка;
...
| `/subscribe` | Сверху — галочки стратегий «Ложный пробой» и «Тренд по недельному каналу» (с 15.09.2026, по умолчанию включены обе), ниже — вкл/выкл подписку по инструменту (кнопки по всем 21 инструменту движка, галочка = подписан). Сигнал приходит, если отмечены и стратегия, и инструмент. Валютные пары в списке ЕСТЬ — с 3 сентября 2026 они снова в движке. |
...
### Таблица `strategy_off` (bot.db) — снятые галочки стратегий
| Поле | Тип | Описание |
|---|---|---|
| user_id | INTEGER | Telegram ID |
| strategy | TEXT | Код из `config.STRATEGIES`: `spring` / `trend` |
| — | UNIQUE(user_id, strategy) | Строка = стратегия выключена; нет строки — включена |
...
### Тесты
`python tests/test_engine.py` — 112 юнит-тестов (с 15.09.2026 в том числе ложный пробой по
правилам 23 июня и галочки стратегий) аналитики, паттернов, ATR и двух фильтров

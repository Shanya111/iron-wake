...
- **Торговый ассистент (VSA + Spring)** по крипте и форексу (через Kraken — там настоящий объём):
  контекстный анализ (тренд D1 + уровни + зоны ликвидности + стакан/DOM), авто-сигналы по паттернам
  ложного пробоя Spring/Upthrust и трекинг их исхода (дошло до цели/стопа) — см. «Торговый движок»
...
- APScheduler (`AsyncIOScheduler`) — фоновые задачи: проверка алертов (5 мин), мониторинг
  сигналов (5 мин), трекинг исхода сигналов (5 мин), контекстный анализ (1 час)
...
├── scheduler.py        — фоновые задачи движка: run_analysis (1ч) + monitor_signals (5м) + track_signals (5м)
...
Единый реестр `INSTRUMENTS` (код → имя, тикер Yahoo, точность, ccxt-символ). Готовых пар 11:
USD/JPY, EUR/USD, GBP/USD, AUD/USD, USD/CAD, Золото (`GC=F`), Нефть Brent (`BZ=F`), Bitcoin,
Solana, Ethereum, Toncoin (`TON11419-USD`). Плюс «своя пара» — любой тикер Yahoo вводится вручную
...
Поле `"ccxt"` (символ + биржа) есть у инструментов с настоящим биржевым объёмом — это вход в
торговый движок (`engine_codes()`). **Все они на одной бирже — Kraken**: крипта (BTC/ETH/SOL/TON)
и форекс (EUR/USD, GBP/USD, AUD/USD, USD/CAD, USD/JPY). Kraken не блокирует РФ (в отличие от
Binance 451 / Bybit 403) и даёт реальный объём и по крипте, и по фиат-парам — поэтому одна биржа
закрывает весь движок без прокси. Без `"ccxt"` (Золото, Нефть, своя пара) инструмент работает
только в простых алертах «касание уровня» (на Kraken-споте фьючерсов на металл/нефть нет).
...
Поверх простых алертов работает торговый ассистент по инструментам с **настоящим биржевым
объёмом** — все на **Kraken**: крипта (BTC/ETH/SOL/TON) и форекс (EUR/USD, GBP/USD, AUD/USD,
USD/CAD, USD/JPY). У Kraken реальный объём и по крипте, и по фиат-парам (в отличие от форекса у
Yahoo, где объёма нет), API бесплатный без ключей, и он не блокирует РФ. OANDA не используем (из
РФ недоступна). Простые алерты «касание уровня» при этом не тронуты — движок живёт отдельными
модулями и таблицами.

### Источник данных — `data_fetcher.py`
CCXT (`ccxt.async_support`), публичные эндпоинты, **без API-ключей**. Биржа и символ заданы по
инструментам в `instruments.py` (поле `"ccxt"`) — для всех это **Kraken** (`BTC/USD`, `TON/USD`,
`EUR/USD`, `USD/JPY` и т.д.). Свечи (`get_candles(symbol, timeframe, limit, exchange)`) кешируются
...
### Стратегия №3 — трекинг исхода сигналов (`pattern_detector.evaluate_signal`)
...
Тот же `AsyncIOScheduler`, что у алертов. Три задачи:
...
- `track_signals(bot)` — каждые 5 минут: исход открытых сигналов (`evaluate_signal`), обновление
  `signals.status` и уведомление подписчиков о достижении цели/стопа. Свечи берёт из общего кеша
  H1 (без лишних запросов к бирже).

`run_analysis`/`monitor_signals` работают по инструментам движка с подпиской (крипта + форекс); `track_signals`
доводит до исхода уже открытые сигналы по любому инструменту (даже если подписку успели снять).

### Команды
| Команда | Что делает |
|---|---|
| `/analyze [код]` | Анализ инструмента: тренд D1 + сильные/слабые уровни + зоны ликвидности + стакан (DOM: давление/спред/стены). Поверх чисел — гибридный AI-разбор через OpenRouter (ошибка LLM не критична). |
| `/subscribe` | Вкл/выкл подписку на сигналы по инструменту (кнопки крипты, галочка = подписан). |
| `/signals` | Последние сигналы с вход/стоп/цель, приоритетом и статусом исхода (⏳ ждём / ✅ цель / 🛑 стоп / ⌛ истёк). |
...
| status | TEXT DEFAULT 'pending' | `pending` / `hit_tp` / `hit_sl` / `expired` — трекинг исхода (`track_signals`) |
...
### Тесты
`python tests/test_engine.py` — 23 юнит-теста аналитики, паттернов, трекинга исхода и стакана
на синтетических данных (сети не требуют, запускаются и под pytest).

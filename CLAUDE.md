...
- **ЛОЖНЫЙ ПРОБОЙ В БОЮ ПО ПРАВИЛАМ 23 ИЮНЯ** (15 сентября 2026, решение владельца,
  ...
  фитилём + 0.1%, цель — ближайший уровень). **Всё, что ниже написано про правила
...
- APScheduler (`AsyncIOScheduler`) — фоновые задачи движка (планировщик один, в `scheduler.py`):
  ...
  тренд по недельному каналу (5 мин), контекстный анализ (1 час)
...
Шесть задач, все в бою: `run_analysis` и `monitor_signals` не ставятся при
...
| `/trend` | **Тренд по недельному каналу** (стратегия №4): открытые позиции модели с текущим итогом в R и границей выхода, закрытые сделки с запуска. R — без комиссии и фандинга. Сигналы входа, стопа и выхода приходят подписчикам инструмента. |
...
| user_id | INTEGER | Telegram ID |
| instrument | TEXT | Код инструмента |
| strategy | TEXT | Код из `config.STRATEGIES`: `spring` / `trend` |
| created_at | TEXT | ISO 8601 |
...
### Тесты
`python tests/test_engine.py` — 117 юнит-тестов (с 15.09.2026 в том числе ложный пробой по

...
торговый движок (`engine_codes()`): крипта (BTC/ETH/SOL — Binance, TON — Bybit) **и форекс через
Kraken** (EUR/USD, GBP/USD, AUD/USD — у Kraken фиат-пары с реальным объёмом, в отличие от форекса
у Yahoo, где объёма нет). Без `"ccxt"` пара работает только в простых алертах «касание уровня».
...
объёмом**: крипта (на бирже объём есть) и форекс через **Kraken** (фиат-пары EUR/USD, GBP/USD,
AUD/USD с реальным объёмом — в отличие от форекса у Yahoo, где объёма нет). OANDA/тиковый объём
не используем (из РФ OANDA недоступна, а у Kraken объём настоящий и API бесплатный без ключей).
Простые алерты «касание уровня» при этом не тронуты — движок живёт отдельными модулями и таблицами.

### Источник данных — `data_fetcher.py`
CCXT (`ccxt.async_support`), публичные эндпоинты бирж, **без API-ключей**. Биржа и символ заданы
по инструментам в `instruments.py` (поле `"ccxt"`): крипта BTC/ETH/SOL — Binance, TON — Bybit;
форекс EUR/USD, GBP/USD, AUD/USD — Kraken (не блокирует РФ, объём настоящий). Свечи
(`get_candles(symbol, timeframe, limit, exchange)`) кешируются в памяти на `config.CACHE_TTL`
(H1 — 5 мин, D1 — 1 час). Стакан заявок (`get_order_book(symbol, limit, exchange)`) — отдельный
короткий кеш `config.ORDERBOOK_TTL` (30 сек, стакан живой). Если биржа блокирует регион
(Binance из РФ → HTTP 451), задаётся `CCXT_PROXY` (прокси в разрешённой стране; http(s):// или socks5://).
...
- `analyze_order_book(ob, wall_mult)` — сводка по стакану (DOM, по инструментам движка — крипта и
  форекс через Kraken): давление сторон (`buyers`/`sellers`/`balance` по дисбалансу объёмов bid/ask),
  спред и «стены» — крупнейшие заявки, чей объём > медианы стороны × `WALL_MULT` (медиана, т.к. сама
  стена раздувает среднее). Запись стакана берётся по индексу — Binance отдаёт `[цена, объём]`, Kraken
  `[цена, объём, время]`. Подмешивается в `/analyze` (текст + AI-промпт); ошибка стакана не валит анализ.
